...
Residents and vendors are public users, not approved staff accounts. Residents enter directly, type their development, submit a complaint without an address requirement, receive an `RC-#####` complaint number, and check status with that complaint number alone. The private status token is device-managed and must never be displayed in app or website UI. Vendors enter their name and the server-issued code Procurement emailed to view a released scope and submit pricing.
