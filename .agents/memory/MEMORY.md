...
- [FIAREP Expo publish startup](fiarep-expo-publish-startup.md) — the missing DevTools libglib warning can be nonfatal; distinguish it from Metro readiness failures.
- [FIAREP mobile spreadsheet security](fiarep-mobile-spreadsheet-security.md) — do not restore the unpatched SheetJS npm packages; keep styled exports compatible with Expo.
...
- [FIAREP client administration boundary](fiarep-client-administration-boundary.md) — Platform Control creates only the first Borough Director; the client manages every later staff change.