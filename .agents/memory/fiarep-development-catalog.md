...
**Why:** Discovering developments from existing assignments creates a circular dependency: a new employee cannot be assigned until a development already appears on another record. A portfolio label alone does not establish property ownership or provide addresses. A valid Jefferson report was excluded from scoring when its development had no registered address row, even though the report's authorized development assignment was correct. Cross-organization catalog controls allowed NYCHA and L+M names to be combined into an invalid portfolio. Public web results can be incomplete or ambiguous, so researched entries must not become authoritative without review.

**How to apply:** Save organization type and development names with the organization, show the resulting portfolio size in Platform Control, and group registered addresses by exact normalized development name. Match built-in catalogs by normalized organization identity, expose only the matching catalog control, and replace rather than merge authoritative lists. For unknown organizations, show research sources and let Platform Control remove incorrect entries before saving. Expose only approved names to tenant staff, and require non-Borough-Director staff accounts to receive at least one approved development assignment. For authorized operational records, calculate the development aggregate from the record's development assignment; use property rows for address registration and display, not as a prerequisite for scoring.