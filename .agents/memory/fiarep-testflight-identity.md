...
Signed FIAREP iOS binaries must use bundle identifier `com.fiarep.app` and target the existing FIAREP App Store Connect application with Apple ID `6811256624`. Use the same Apple Developer team that owns that application. The user confirmed the next TestFlight build number is build 5.
