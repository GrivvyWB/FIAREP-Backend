...
**How to apply:** Preserve the original work record and link it to a separate tenant-scoped request. Use canonical supervisor and employee IDs, match the employee to the requested trade and development, notify the receiver, filter trade-supervisor reads through the linked request or direct assignment, and update the source assignment only during an authorized dispatch.