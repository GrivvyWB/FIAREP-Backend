...
Emergency truck drivers receive sequential TRK labels such as TRK-1 and TRK-2. Their generated sign-in code combines the truck number with a random suffix, such as TRK1-956G, while their login name remains their normal employee name. Onboarding creates a linked emergency-unit record owned by the canonical staff ID. Emergency Maintenance employees who do not drive emergency trucks receive neither a TRK label nor a truck-specific code. All members appear beneath Superintendent Ⓔ in the Team hierarchy.
...
**How to apply:** Use the Emergency role plus Maintenance Worker position as the authoritative membership rule. In authorization and navigation, inherit ordinary Maintenance Worker capabilities and add emergency-job access. When HR completes a truck-driver intake, atomically assign the next TRK number, generate a unique truck-prefixed code, and create the linked unit. Replacement-code issuance generates a new suffix and repairs missing links for legacy truck drivers. A TRK designation is not a separate role.