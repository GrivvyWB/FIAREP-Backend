...
export default function Inspections() {
  const { data: inspections, isLoading } = useListEntityRecords("inspections");
  const [search, setSearch] = useState("");

  const filtered = inspections?.filter(i => {
    if (!search) return true;
    const title = (i.state as any)?.title?.toLowerCase() || "";
    const dev = i.development?.toLowerCase() || "";
    const q = search.toLowerCase();
    return title.includes(q) || dev.includes(q);
  });
        ...
        <div className="p-4">
          {isLoading ? (
            ...
            <div className="grid gap-3">
              {filtered?.map(insp => (
                <div key={insp.id} className="flex items-center gap-4 p-4 rounded-xl border border-border hover:bg-muted/30 transition-colors">
                  <div className="w-12 h-12 rounded-[9px] bg-secondary text-secondary-foreground grid place-items-center shrink-0">
                    <Building2 className="w-6 h-6" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-[15px] truncate">{(insp.state as any)?.title || "Untitled Inspection"}</h4>
