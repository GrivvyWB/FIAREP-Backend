...
router.post("/v1/platform/organizations/:id/director-transfer", async (req, res) => {
  const organizationId = req.params.id!;
  const name = typeof req.body?.name === "string" ? req.body.name.trim() : "";
  if (!name) {
    res.status(400).json({ error: "Borough Director name is required" });
    return;
  }
  const owner = res.locals["platformOwner"] as { name: string };
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (targets.length > 1) {
        throw Object.assign(new Error("Multiple staff accounts use that name"), { status: 409 });
      }
      ...
      const replaced = directors.filter((director) => director.id !== target.id);
      const replacedIds = replaced.map((director) => director.id);
      ...
      if (replacedIds.length > 0) {
        await tx
          ...
          .where(inArray(staffAccounts.id, replacedIds));
      }
      ...
      const invalidatedStaffIds = [...new Set([target.id, ...replacedIds])];
      await tx
        .delete(refreshSessions)
        .where(inArray(refreshSessions.staffId, invalidatedStaffIds));

      return { promoted: promoted!, target, replaced };
    });
    await platformAudit(
      owner.name,
      "organization.director_transferred",
      organizationId,
      { target: result.target, replaced: result.replaced },
      result.promoted,
    );
    res.json({
      id: result.promoted.id,
      name: result.promoted.name,
      tenantId: result.promoted.tenantId,
      code: result.promoted.code,
    });
  } catch (error: any) {
    if (error?.status) {
      res.status(error.status).json({ error: error.message });
